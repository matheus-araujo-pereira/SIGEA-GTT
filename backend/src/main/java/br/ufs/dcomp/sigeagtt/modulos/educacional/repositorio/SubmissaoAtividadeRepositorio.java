...
public interface SubmissaoAtividadeRepositorio extends JpaRepository<SubmissaoAtividade, Long> {
    ...
    @Query(
            "SELECT s FROM SubmissaoAtividade s WHERE s.atividade.turma.professorResponsavel.id = :professorId AND s.status = :status ORDER BY s.dataSubmissao ASC")
    List<SubmissaoAtividade> findByProfessorAndStatus(
            @Param("professorId") Long professorId, @Param("status") StatusSubmissao status);

    @Query(
            "SELECT s FROM SubmissaoAtividade s WHERE s.atividade.turma.professorResponsavel.id = :professorId ORDER BY s.dataSubmissao DESC")
