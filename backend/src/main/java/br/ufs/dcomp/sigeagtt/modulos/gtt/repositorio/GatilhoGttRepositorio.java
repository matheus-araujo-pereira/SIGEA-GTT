...
public interface GatilhoGttRepositorio extends JpaRepository<GatilhoGtt, Long> {
    ...
    @Query("SELECT g FROM GatilhoGtt g JOIN FETCH g.modulo WHERE g.modulo.id = :moduloId ORDER BY g.codigo ASC")
    List<GatilhoGtt> findAllByModuloId(@Param("moduloId") Long moduloId);

    @Query("SELECT g FROM GatilhoGtt g JOIN FETCH g.modulo ORDER BY g.codigo ASC")
