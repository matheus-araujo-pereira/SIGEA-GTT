...
public class SubmissaoAtividadeServico {
    ...
    public List<MinhaAtividadeItemDTO> listarMinhasAtividades(Usuario alunoLogado) {
        List<AtividadeEducacional> atividades =
                atividadeRepositorio.findAtividadesParaAluno(alunoLogado.getId());
        List<SubmissaoAtividade> minhasSubs =
                submissaoRepositorio.findByAlunoIdOrderByDataInicioDesc(alunoLogado.getId());
    ...
    public SubmissaoDTO iniciarOuContinuar(Long atividadeId, Usuario alunoLogado) {
        AtividadeEducacional atividade =
                atividadeRepositorio
                        ...
                        .orElseThrow(
                                () -> new IllegalArgumentException("Atividade não encontrada"));
    ...
    public SubmissaoDTO buscarSubmissao(Long submissaoId, Usuario usuarioLogado) {
        SubmissaoAtividade sub =
                submissaoRepositorio
                        .findById(submissaoId)
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "Submissão não encontrada (ID: "
                                                        + submissaoId
                                                        + ")"));
    ...
    @Transactional
    public SubmissaoDTO salvarProgresso(
            Long submissaoId, SalvarSubmissaoDTO dto, Usuario alunoLogado) {
        SubmissaoAtividade sub =
                submissaoRepositorio
                        .findById(submissaoId)
                        .orElseThrow(
                                () -> new IllegalArgumentException("Submissão não encontrada"));

        if (!sub.getAluno().getId().equals(alunoLogado.getId())) {
            throw new IllegalArgumentException(
                    "Você não tem permissão para alterar esta submissão.");
        }
        ...
        if (Boolean.TRUE.equals(dto.finalizar())) {
            ...
            if (dto.ishikawa() == null
                    || dto.ishikawa().efeitoPrincipal() == null
            ...
            if (dto.pdca() == null
                    || dto.pdca().planejar() == null
                    || dto.pdca().planejar().trim().isEmpty()
                    || dto.pdca().fazer() == null
                    || dto.pdca().fazer().trim().isEmpty()
                    || dto.pdca().checar() == null
                    || dto.pdca().checar().trim().isEmpty()
                    || dto.pdca().agir() == null
                    || dto.pdca().agir().trim().isEmpty()) {
                throw new IllegalArgumentException(
        ...
        if (dto.achadosGatilhos() != null) {
            ...
            for (SubmissaoGatilhoDTO aDto : dto.achadosGatilhos()) {
                GatilhoGtt gat =
                        gatilhoRepositorio
                                .findById(aDto.gatilhoId())
                                .orElseThrow(
                                        () ->
                                                new IllegalArgumentException(
                                                        "Gatilho inválido: " + aDto.gatilhoId()));

                CategoriaEventoAdverso cat =
                        aDto.categoriaEaId() != null
        ...
        // 2. Salvar Ishikawa
        if (dto.ishikawa() != null) {
            SubmissaoIshikawa ishikawa =
                    submissaoIshikawaRepositorio
                            .findBySubmissaoId(sub.getId())
                            .orElseGet(
                                    () -> {
    ...
    @Transactional
    public SubmissaoDTO avaliar(
            Long submissaoId, AvaliarSubmissaoDTO dto, Usuario professorLogado) {
        SubmissaoAtividade sub =
                submissaoRepositorio
                        .findById(submissaoId)
                        .orElseThrow(
        ...
        if (professorLogado.getPerfil() != PerfilUsuario.ADMINISTRADOR
            ...
            throw new IllegalArgumentException(
                    "Você não tem permissão para avaliar submissões desta turma.");
        }
    ...
    public List<SubmissaoDTO> listarPendentesCorrecao(Usuario professorLogado) {
        ...
        if (professorLogado.getPerfil() == PerfilUsuario.ADMINISTRADOR) {
            pendentes =
                    submissaoRepositorio.findAll().stream()
                            .filter(s -> s.getStatus() == StatusSubmissao.SUBMETIDA)
                            .toList();
        } else {
            pendentes =
                    submissaoRepositorio.findByProfessorAndStatus(
                            professorLogado.getId(), StatusSubmissao.SUBMETIDA);
        }
    ...
    private SubmissaoDTO converterParaDTO(SubmissaoAtividade s) {
        List<SubmissaoGatilho> gatilhos =
                s.getAchadosGatilhos() != null
                        ? s.getAchadosGatilhos()
                        : submissaoGatilhoRepositorio.findBySubmissaoId(s.getId());
        List<SubmissaoGatilhoDTO> gatilhosDTO =
                gatilhos.stream()
                        .map(
                                g ->
                                        new SubmissaoGatilhoDTO(
                                                g.getId(),
                                                g.getGatilho().getId(),
                                                g.getGatilho().getCodigo(),
                                                g.getGatilho().getDescricao(),
                                                g.getGatilho().getModulo() != null
                                                        ? g.getGatilho().getModulo().getCodigo()
                                                        : null,
                                                g.getGatilho().getModulo() != null
                                                        ? g.getGatilho().getModulo().getNome()
                                                        : null,
                                                g.getCategoriaEa() != null
                                                        ? g.getCategoriaEa().getId()
                                                        : null,
                                                g.getCategoriaEa() != null
                                                        ? g.getCategoriaEa().getNome()
                                                        : null,
                                                g.getConfirmouDano(),
        ...
        SubmissaoIshikawaDTO ishikawaDTO =
                ishikawa != null
                        ? new SubmissaoIshikawaDTO(
        ...
        List<SubmissaoPlano5w3h> planos =
                s.getPlanos5w3h() != null
                        ? s.getPlanos5w3h()
                        : submissaoPlano5w3hRepositorio.findBySubmissaoId(s.getId());
        List<SubmissaoPlano5w3hDTO> planosDTO =
                planos.stream()
                        .map(
                                p ->
                                        new SubmissaoPlano5w3hDTO(
                                                p.getId(),
                                                p.getOQue(),
                                                p.getPorQue(),
                                                p.getQuem(),
        ...
        SubmissaoPdcaDTO pdcaDTO =
                pdca != null
                        ? new SubmissaoPdcaDTO(
                                pdca.getPlanejar(),
                                pdca.getFazer(),
                                pdca.getChecar(),
                                pdca.getAgir())
